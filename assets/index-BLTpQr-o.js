(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))n(i);new MutationObserver(i=>{for(const a of i)if(a.type==="childList")for(const o of a.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&n(o)}).observe(document,{childList:!0,subtree:!0});function l(i){const a={};return i.integrity&&(a.integrity=i.integrity),i.referrerPolicy&&(a.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?a.credentials="include":i.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function n(i){if(i.ep)return;i.ep=!0;const a=l(i);fetch(i.href,a)}})();const p="/CUOpenDayTest/cu-logo.svg";async function b(){return await(await fetch("/CUOpenDayTest/api/OpenDay.json")).json()}const m=[];function x(){const s=document.getElementById("modal-title"),e=document.getElementById("modal-programs");s.textContent="Bookmarked Programs",e.innerHTML=m.length>0?`
          ...
          <li class="px-4 py-4 sm:px-0">
            <div class="flex flex-col gap-2">
              ...
              <p class="text-sm text-gray-600">
                ${new Date(n.start_time).toLocaleTimeString("en-GB")} - ${new Date(n.end_time).toLocaleTimeString("en-GB")}
              </p>
              <p class="text-sm text-gray-600">Location: ${n.location_title}</p>
              <button class="text-red-600 text-sm font-medium" data-remove-bookmark="${n.title}">Remove Bookmark</button>
            </div>
          </li>
        `).join("")}
      </ul>
    `:"<p>No bookmarked programs.</p>",document.getElementById("modal").classList.remove("hidden")}async function v(){return(await(await fetch("/CUOpenDayTest/api/Locations.json")).json()).university}function h(s,e){const l=e.find(n=>n.id===s);return l?l.title:"Unknown Location"}function w(s){const e=document.querySelector("#base");if(!s.topics){e.innerHTML='<p class="text-red-600">No Open Day data found.</p>';return}const l=new Date(s.start_time).toLocaleString("en-GB",{weekday:"long",year:"numeric",month:"long",day:"numeric",hour:"2-digit",minute:"2-digit"}),n=new Date(s.end_time).toLocaleTimeString("en-GB",{hour:"2-digit",minute:"2-digit"});e.innerHTML=`
      ...
      <div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white px-4 py-8 sm:py-16 lg:px-6">
          <div class="mx-auto max-w-2xl text-center">
            <p class="text-base/7 font-semibold text-cardiff-red">${l} - ${n}</p>
  ...
  `,document.querySelector("#search").addEventListener("input",a=>{const o=a.target.value.toLowerCase(),t=s.topics.filter(c=>c.name.toLowerCase().includes(o)),r=document.querySelector("#topics-container");t.length>0?r.innerHTML=y(t):r.innerHTML='<p class="text-gray-600 text-center mt-4 mb-8">No topics match your search. Please try a different keyword.</p>'}),document.addEventListener("click",async a=>{const o=a.target;if(o.matches("button[data-topic-name]")){const t=o.getAttribute("data-topic-name"),r=document.getElementById("modal"),c=document.getElementById("modal-title"),u=document.getElementById("modal-programs"),f=await v(),g=s.topics.find(d=>d.name===t);c.textContent=`${t} Events`,u.innerHTML=g.programs&&g.programs.length>0?`
          <ul role="list" class="divide-y divide-gray-200">
            ${g.programs.map(d=>`
              <li class="px-4 py-4 sm:px-0">
                <div class="flex items-center gap-4">
                  ...
                  <div class="flex flex-col">
                    <p class="text-sm text-gray-600">
                      ${h(d.location_id,f)}, <i>${new Date(d.start_time).toLocaleTimeString("en-GB")} - ${new Date(d.end_time).toLocaleTimeString("en-GB")}</i>
                    </p>
        ...
        `:"<p>No programs available for this topic.</p>",r.classList.remove("hidden")}if(o.matches("button[data-toggle-details]")){const t=o.closest("li")?.querySelector(".program-details"),r=o.closest("li")?.querySelector(".bookmark-button");t&&(t.classList.toggle("hidden"),o.textContent=t.classList.contains("hidden")?"+":"-",r&&r.classList.toggle("hidden",t.classList.contains("hidden")),o.classList.toggle("active",!t.classList.contains("hidden")))}if(o.matches("button[data-bookmark-program]")){const t=JSON.parse(o.getAttribute("data-bookmark-program")),r=await v(),c=h(t.location_id,r),u={...t,location_title:c};m.some(f=>f.title===t.title)?alert(`Program "${t.title}" is already bookmarked.`):(m.push(u),alert(`Program "${t.title}" bookmarked!`))}if(o.matches("button[data-remove-bookmark]")){const t=o.getAttribute("data-remove-bookmark"),r=m.findIndex(c=>c.title===t);r!==-1&&(m.splice(r,1),x())}o.matches("button#view-bookmarks")&&x()}),document.getElementById("close-modal").addEventListener("click",()=>{document.getElementById("modal").classList.add("hidden")})}function y(s){return s.map(e=>e&&e.name?`
