(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))n(i);new MutationObserver(i=>{for(const s of i)if(s.type==="childList")for(const t of s.addedNodes)t.tagName==="LINK"&&t.rel==="modulepreload"&&n(t)}).observe(document,{childList:!0,subtree:!0});function l(i){const s={};return i.integrity&&(s.integrity=i.integrity),i.referrerPolicy&&(s.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?s.credentials="include":i.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function n(i){if(i.ep)return;i.ep=!0;const s=l(i);fetch(i.href,s)}})();const p="/Open-Day-2/cu-logo.svg";async function y(){return await(await fetch("/Open-Day-2/api/OpenDay.json")).json()}const u=[];function x(){const a=document.getElementById("modal-title"),e=document.getElementById("modal-programs");a.textContent="Bookmarked Programs",e.innerHTML=u.length>0?`
        ...
        ${u.map(n=>`
          <li class="px-4 py-4 sm:px-0">
            <div class="flex flex-col gap-2">
              ...
              <button class="text-red-600 text-sm font-medium" data-remove-bookmark="${n.title}" aria-label="Remove bookmark for ${n.title}">Remove Bookmark</button>
            </div>
          </li>
        `).join("")}
      </ul>
    `:"<p>No bookmarked programs.</p>",document.getElementById("modal").classList.remove("hidden")}async function v(){return(await(await fetch("/Open-Day-2/api/Locations.json")).json()).university}function h(a,e){const l=e.find(n=>n.id===a);return l?l.title:"Unknown Location"}function w(a){const e=document.querySelector("#base");if(!a.topics){e.innerHTML='<p class="text-red-600">No Open Day data found.</p>';return}const l=new Date(a.start_time).toLocaleString("en-GB",{weekday:"long",year:"numeric",month:"long",day:"numeric",hour:"2-digit",minute:"2-digit"}),n=new Date(a.end_time).toLocaleTimeString("en-GB",{hour:"2-digit",minute:"2-digit"});e.innerHTML=`
    ...
    <div>
      <div>
        <img class="h-60 w-full object-cover lg:h-60" src="${a.cover_image}" alt="" />
      </div>
                  ...
                  <div class="flex flex-col">
                    ...
                    <div class="mt-4 hidden text-sm text-gray-600 program-details">
                      ...
                      <div class="bookmark-action mt-4 flex items-center gap-2">
                        <button class="bookmark-button text-cardiff-red text-sm font-medium hover:border-cardiff-red flex items-center gap-2" data-bookmark-program="${encodeURIComponent(JSON.stringify(d))}" aria-label="Bookmark this program">
                          <svg class="size-6" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M6.75 3a.75.75 0 0 0-.75.75v16.5a.25.25 0 0 0 .4.2L12 17.25l5.6 3.2a.25.25 0 0 0 .4-.2V3.75a.75.75 0 0 0-.75-.75H6.75z" />
                          </svg>
                          Bookmark
